...
$email = $username = $address = $password = $confirm_password = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        $sql = "SELECT P.place_email FROM Places P WHERE P.place_email = ?";
    ...
    if (empty($email_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO Places (place_id, place_name, place_address, place_email, place_password) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            $param_visitor_id = generateKey($link);
            mysqli_stmt_bind_param($stmt, "dssss", $param_place_id, $param_username, $param_address, $param_email, $param_password);
        ...
        <div class="jumbotron my-auto">
            <div class="wrapper">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <h9>Already have an account? <a href="/~bassefa/paths/places/places_login.php">Login here</a>.</h9>
                </form>
