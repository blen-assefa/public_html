...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        $sql = "SELECT H.hospital_username FROM Hospital H WHERE H.hospital_username = ?";
    ...
    if (empty($username_err) && empty($address_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO Hospital (hospital_id, hospital_username, hospital_address, hospital_password) VALUES (?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            $param_visitor_id = generateKey($link);
            mysqli_stmt_bind_param($stmt, "dsss", $param_hospital_id, $param_username, $param_address, $param_password);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $_SESSION["huser"] = $param_username;
                $_SESSION["hospital_id"] = $param_hospital_id;

                // Redirect user to welcome page
                header("location: /paths/hospitals/hospitals_dashboard.php");
            } else {
